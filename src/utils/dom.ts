class Dom {
   ...
   */
  getDisplayNoneOffset(el: Element) {
    let height = 0;
    let width = 0;
    if (!el || el.nodeType !== 1) return { height, width };

    // 拷贝子元素
    let clone: any = el.cloneNode(true);
   ...
   *
   */
  getScrollToBottom(toBottomNum: number, callback: Function) {
    const scrollObj = {
    ...
    window.onscroll = function() {
      if (
        scrollObj.getScrollHeight() -
          (scrollObj.getScrollTop() + scrollObj.getWindowHeight()) <=
        toBottomNum
      ) {
   ...
   * 获取DOM元素到文档顶部的距离
   */
  getElementToPageTop(el: any): any {
    if (!el) return 0;
    if (el.offsetParent !== document.body) {
      const marginTop = ~~el.style.marginTop.slice(0, -2);
      return (
        this.getElementToPageTop(el.offsetParent) + el.offsetTop + marginTop
      );
    }
    return el.offsetTop;
  }
  ...
  // 设置class
  setClass(el: HTMLElement, cls: string) {
    if (!el) return;
    return el.setAttribute('class', cls);
  }

  // 判断class是否存在
  hasClass(elements: HTMLElement, cName: string) {
    if (!elements) return;
  ...
  // 添加clss
  addClass(elements: HTMLElement, cName: string) {
    if (!elements) return;
  ...
  // 删除class
  removeClass(elements: HTMLElement, cName: string) {
    if (!elements) return;
    if (this.hasClass(elements, cName)) {
  ...
  // 切换class
  toggleClass(elements: HTMLElement, cName: string) {
    if (!elements) return;
    if (this.hasClass(elements, cName)) {
      elements.className = elements.className.replace(
        new RegExp('(\\s|^)' + cName + '(\\s|$)'),
